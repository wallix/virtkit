...
//! through the hybrid vsock-mux with `virtkit-agent connect` as the ssh ProxyCommand.
//!
...
pub async fn run_ssh_server(
    ...
    info!(
        "virtkit-agent ssh: listening on {socket} ({} authorized key(s))",
        keys.len()
    );
...
impl Handler for ServerHandler {
    ...
    /// Subsystem request — we serve `sftp` by spawning `virtkit-agent sftp-server` as
