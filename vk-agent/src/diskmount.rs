...
//! over the existing exec channel as `virtkit-agent mount|umount …`, like `fsfreeze`.
...
/// CLI entry for `virtkit-agent mount|umount|copy …`. Returns the process exit code.
...
fn copy_cmd(mut args: &[String]) -> Result<()> {
    ...
    if args.len() < 2 {
        bail!(
            "usage: virtkit-agent copy [--chown u:g] [--chmod OCTAL] [--ignore-root DIR] <src>... <dst>"
        );
    }
...
fn usage(msg: &str) -> i32 {
    eprintln!("usage: virtkit-agent {msg}");
    2
}
